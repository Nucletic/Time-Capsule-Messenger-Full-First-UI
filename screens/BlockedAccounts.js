...
const BlockedAccounts = ({ navigation }) => {
  ...
  const getBlockedAccounts = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.29.8:5000/users/getBlockedAccounts`, {
        method: 'GET',
        credentials: 'include',
        headers: {
  ...
  const unblockUser = async (userId) => {
    try {
      const idToken = await FIREBASE_AUTH.currentUser.getIdToken();
      const encryptedIdToken = encryptData(idToken, SECRET_KEY);
      const response = await fetch(`http://192.168.29.8:5000/users/unblockUser/${userId}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: {
          'Authorization': 'Bearer ' + encryptedIdToken,
  ...
  return (
    <View style={styles.Container}>
      ...
      {/* <Pressable onPress={() => { navigation.navigate('BlockedSearch') }} style={styles.SearchBox}> */}
      <Pressable style={styles.SearchBox}>
        <View style={styles.MainSearchBoxContainer}>
          ...
          {/* <Text style={styles.MainSearchBoxText}>Search Blocked Accounts</Text> */}
