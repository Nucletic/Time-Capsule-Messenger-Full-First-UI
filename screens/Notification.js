...
const Notification = ({ navigation }) => {
  ...
  const getAllNotifications = async () => {
    try {
      ...
      // const response = await fetch(`http://10.0.2.2:5000/users/${CustomUUID}/getFollowRequestNotifications`, {
  ...
  const AcceptChatmateRequest = async (requestIndex, ReciverUUID, SenderUUID) => {
    try {
      ...
      const idToken = await FIREBASE_AUTH.currentUser.getIdToken();
      const encryptedIdToken = encryptData(idToken, SECRET_KEY);
      // const response = await fetch(`http://10.0.2.2:5000/users/AcceptChatmateRequest`, {
