...
import { collection, doc, getDoc, Timestamp, setDoc, addDoc, updateDoc, serverTimestamp, query, where } from 'firebase/firestore';
  ...
  const LikeTale = async () => {
    try {
      ...
      ws.send(JSON.stringify({
        type: 'likedtale',
        idToken: encryptedIdToken,
        LikerUUID: CustomUUID,
        TaleOwnerUUID: data.userId,
      }));
    } catch (error) {
      throw new Error(error);
    }
        ...
        <View style={styles.TaleTopContent}>
          ...
          <View style={styles.PlayNameContainer}>
            <View style={styles.NameContainer}>
              <Image source={require('../assets/Images/user2.jpg')} style={styles.UserProfilePicture} />
